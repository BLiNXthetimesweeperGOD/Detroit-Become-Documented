...
class fileTools:
    ...
    def scanForAllInstancesOfBytes(path, string):
        """
        Reads an entire file into memory and checks it for all instances of a byte string
        
        Returns: Tuple of (bool, list of offsets)
            - bool: True if any instances found, False otherwise
            - list: Contains all offsets where string was found (empty if none found)
        ...
        with open(path, 'rb') as file:
            ...
            start = 0
            while True:
                offset = content.find(string, start)
                if offset == -1:  # No more occurrences
                    break
                offsets.append(offset)
                start = offset + 1  # Move past the current occurrence
